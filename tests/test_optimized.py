...
class TestOptimized(TestMethods):
    ...
    def test__unbiasedness_constraints(self):
        ...
        r = il.Ranking([1, 2])
        r.credits = {
            0: {1: 1.0, 2: 0.5}, 1: {1: 1.0, 2: 0}, 2: {1: 1.0, 2: 0}}
        rankings.append(r)
        r = il.Ranking([1, 3])
        r.credits = {
            0: {1: 1.0, 3: 0}, 1: {1: 1.0, 3: 0.5}, 2: {1: 1.0, 3: 0}}
        rankings.append(r)
        r = il.Ranking([1, 4])
        r.credits = {
            0: {1: 1.0, 4: 0}, 1: {1: 1.0, 4: 0}, 2: {1: 1.0, 4: 0.5}}
        rankings.append(r)
        ...
        assert res[1, 0] == 0.5 # Rank 2, Ranking 1, System 1 - System 2
        assert res[1, 1] == -0.5 # Rank 2, Ranking 2, System 1 - System 2
        ...
        assert res[3, 1] == 0.5 # Rank 2, Ranking 2, System 2 - System 3
        assert res[3, 2] == -0.5 # Rank 2, Ranking 3, System 2 - System 3
    ...
    def test__sensitivity(self):
        ...
        rankings = []
        r = il.Ranking([1, 2])
        r.credits = {0: {1: 1.0, 2: 0.5}, 1: {1: 0.0, 2: 1.0}}
        rankings.append(r)
        r = il.Ranking([2, 1])
        r.credits = {0: {1: 1.0, 2: 0.5}, 1: {1: 0.0, 2: 1.0}}
        rankings.append(r)
        r = il.Ranking([2, 3])
        r.credits = {0: {2: 0.5, 3: 0.0}, 1: {2: 1.0, 3: 0.5}}
        rankings.append(r)
        res = b._sensitivity(lists, rankings)
        assert len(res) == 3
        assert res[0] == ((1 + 0.5 * 0.5 - 1.75/2) ** 2 + (0.5 - 1.75/2) ** 2)
        assert res[1] == ((0.5 + 0.5 - 1.0) ** 2 + (1.0 - 1.0) ** 2)
        assert res[2] == ((0.5 - 1.75/2) ** 2 + (1.0 + 0.5 * 0.5 - 1.75/2) ** 2)
    ...
    def test__compute_probabilities(self):
        ...
        self.assert_almost_equal(np.inner([1, -0.5, -0.5], p), 0)
        self.assert_almost_equal(np.inner([0.5, 0.5, -1], p), 0)
        self.assert_almost_equal(p[0], 1.0/3)
        self.assert_almost_equal(p[1], 1.0/3)
        self.assert_almost_equal(p[2], 1.0/3)
    ...
    def test_interleave(self):
        ...
        for i in range(len(probabilities)):
            self.assert_almost_equal(probabilities[i], 1.0/3)
        ...
        for r, c in counts.items():
            self.assert_almost_equal(float(c)/trials, 1.0/3)

...
class TeamDraftRanking(Ranking):
    '''
    A list of document IDs generated by TeamDraft, including teams
    '''
    def __hash__(self):
        '''
        TeamDraftRanking can be a key by which
        rankings with the same document ID list
        and the same team assignment are the same
        '''
        ranker_ids = sorted(self.teams.keys())
        team_tuple = tuple([(i, tuple(sorted(list(self.teams[i]))))
            for i in ranker_ids])
        return hash((tuple(self), team_tuple))
